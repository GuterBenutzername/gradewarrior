...
class DbOps:
    ...
    def get_by_foreign_key(table, foreign_key, value):
        return db.execute_query(
            f"SELECT * FROM {table} WHERE {foreign_key} = ?", (value,)
        )
...
def resolve_create_user(_, info, input):
    user_id = db.execute_command(
        "INSERT INTO users (name) VALUES (?)", (input["name"],)
    )
...
def resolve_create_course(_, info, input):
    course_id = db.execute_command(
        "INSERT INTO courses (name) VALUES (?)", (input["name"],)
    )
...
def resolve_add_assignment_to_course(_, info, courseId, assignmentId):
    ...
    db.execute_command(
        "UPDATE assignments SET course_id = ? WHERE id = ?", (course_id, assignment_id)
    )
